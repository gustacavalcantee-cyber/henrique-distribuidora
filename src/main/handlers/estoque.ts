...
export function registerEstoqueHandlers() {
  ipcMain.handle(IPC.ESTOQUE_QUANTIDADES_DIA, async (_event, data: string, produtoIds: number[]) => {
    if (!produtoIds || produtoIds.length === 0) return {}

    const db = getDb()

    // Get all pedidos for the date
    const pedidosDia = await db.select().from(pedidos).where(eq(pedidos.data_pedido, data))
