...
export function registerLojasHandlers() {
  ...
  ipcMain.handle(IPC.LOJAS_CREATE, (_event, data: { rede_id: number; nome: string; codigo?: string }) => {
    return getDb().insert(lojas).values(data).returning().all()[0]
  })

  ipcMain.handle(IPC.LOJAS_UPDATE, (_event, data: { id: number; nome?: string; codigo?: string; ativo?: number }) => {
