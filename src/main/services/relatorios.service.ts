...
export function getRelatorioQuinzena(rede_id: number, loja_id: number, mes: number, ano: number, quinzena: 1 | 2): QuinzenaSummary {
  ...
  for (const p of [...todosProdutos].sort((a, b) => a.id - b.id)) {
    if (!seenNomes.has(p.nome)) { seenNomes.add(p.nome); produtosUnicos.push(p) }
  }
...
export function getRelatorioCobranca(
  ...
  let data_inicio: string, data_fim: string, periodo_str: string
  if (periodo === '1') {
    data_inicio = `${ano}-${mesStr}-01`; data_fim = `${ano}-${mesStr}-15`; periodo_str = `DE 01 A 15/${mesStr}`
  } else if (periodo === '2') {
    data_inicio = `${ano}-${mesStr}-16`; data_fim = `${ano}-${mesStr}-${lastDay}`; periodo_str = `DE 16 A ${lastDay}/${mesStr}`
  } else {
  ...
  return loja_ids.map(loja_id => {
    ...
    const pedidoIds = pedidosList.map(p => p.id)
    const total_venda = pedidoIds.length > 0
      ? pedidoIds.flatMap(pid => db.select().from(itensPedido).where(eq(itensPedido.pedido_id, pid)).all())
    ...
    return { loja_id, loja_nome: redeName ? `${redeName} ${lojaName}` : lojaName, periodo_str, total_venda }
  })
}

export function getRelatorioPorProduto(
  rede_id: number, produto_ids: number[], mes: number, ano: number,
  periodo: '1' | '2' | 'mes', agrupar_por: 'loja' | 'franqueado'
  ...
  if (periodo === '1') { data_inicio = `${ano}-${mesStr}-01`; data_fim = `${ano}-${mesStr}-15` }
  else if (periodo === '2') { data_inicio = `${ano}-${mesStr}-16`; data_fim = `${ano}-${mesStr}-${lastDay}` }
  else { data_inicio = `${ano}-${mesStr}-01`; data_fim = `${ano}-${mesStr}-${lastDay}` }

  const pedidosList = db.select().from(pedidos).where(
    and(eq(pedidos.rede_id, rede_id), gte(pedidos.data_pedido, data_inicio), lte(pedidos.data_pedido, data_fim))
  ).all()
  ...
  return produto_ids.map(produto_id => {
    ...
    for (const item of itensDoP) {
      ...
      groupMap.set(groupName, { quantidade: prev.quantidade + item.quantidade, valor: prev.valor + item.quantidade * item.preco_unit })
    }
    ...
    return {
      produto_id, produto_nome: produto?.nome ?? String(produto_id), unidade: produto?.unidade ?? '',
      linhas, total_quantidade: linhas.reduce((s, l) => s + l.quantidade, 0), total_valor: linhas.reduce((s, l) => s + l.valor, 0),
    }
  })
}
...
export function getNotasMes(mes: number, ano: number, rede_id?: number, franqueado_id?: number): NotaPagamento[] {
  ...
  const conditions: ReturnType<typeof gte>[] = [gte(pedidos.data_pedido, data_inicio), lte(pedidos.data_pedido, data_fim)]
  ...
  const pedidoIds = pedidosList.map(p => p.id)
  const allItens = pedidoIds.flatMap(pid => db.select().from(itensPedido).where(eq(itensPedido.pedido_id, pid)).all())
  const todasLojas = db.select().from(lojas).all()
  const todasRedes = db.select().from(redes).all()
  const todosFranqueados = db.select().from(franqueados).all()
  ...
  return pedidosList.map(pedido => {
    ...
    return {
      pedido_id: pedido.id, loja_id: pedido.loja_id!, loja_nome: redeName ? `${redeName} ${lojaName}` : lojaName,
      loja_nome_only: lojaName, rede_nome: redeName, franqueado_nome: franqueado?.nome ?? null,
      data_pedido: pedido.data_pedido, numero_oc: pedido.numero_oc, total_venda, status_pagamento: pedido.status_pagamento ?? 'aberto',
    }
  })
}
...
export function getRelatorioPrecoVsCusto(produto_id: number, loja_id?: number): PrecoVsCustoResult {
  ...
  const produto = db.select().from(produtos).where(eq(produtos.id, produto_id)).limit(1).all()[0]
  const produto_nome = produto?.nome ?? String(produto_id)

  const historico_custos: PrecoVsCustoCusto[] = db.select().from(custosTable).where(eq(custosTable.produto_id, produto_id)).all()
  ...
  const precosVigentes = db.select().from(precosTable)
  ...
  const comparacao_lojas: PrecoVsCustoLoja[] = todasLojas.map(loja => {
    ...
    const margem_pct = preco_venda != null && margem_reais != null && preco_venda > 0
      ? (margem_reais / preco_venda) * 100 : null
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const custoDoMes = historico_custos.find(c =>
      c.vigencia_inicio <= lastDay && (c.vigencia_fim === null || c.vigencia_fim >= firstDay)
    )
    ...
    const preco_medio = precosDoMes.length > 0
      ? precosDoMes.reduce((s, p) => s + p.preco_venda, 0) / precosDoMes.length : null
    const margem_pct = preco_medio != null && custoMes != null && preco_medio > 0
      ? ((preco_medio - custoMes) / preco_medio) * 100 : null

    const pedidosDoMes = db.select().from(pedidos).where(
      and(gte(pedidos.data_pedido, firstDay), lte(pedidos.data_pedido, lastDay),
        ...(loja_id ? [eq(pedidos.loja_id, loja_id)] : []))
    ).all()

    const pedidoIds = pedidosDoMes.map(p => p.id)
    const allItensDoMes = pedidoIds.length > 0
      ? db.select().from(itensPedido).where(and(inArray(itensPedido.pedido_id, pedidoIds), eq(itensPedido.produto_id, produto_id))).all()
      : []
    ...
    for (const item of allItensDoMes) {
      ...
      diaMap.set(ped.data_pedido, { custo_sum: prev.custo_sum + item.custo_unit, preco_sum: prev.preco_sum + item.preco_unit, count: prev.count + 1 })
    }
