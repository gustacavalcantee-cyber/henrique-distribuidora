...
// ── Batch print: multiple pedidos in a single print job ────────────────────
// Reuses the same layout as generatePrintHtml — one A4-landscape page per
// pedido, with a page break between them so the printer emits one sheet per
// pedido and the user gets everything with a single Ctrl+P.

export function generateBatchPrintHtml(dataArray: PrintData[], preview = false): string {
  const pages = dataArray.map(data => {
    const dataRows = data.linhas.map(l => `
    <tr>
      <td class="c-prod">${l.nome}</td>
      <td class="c-qty">${fmtQty(l.quantidade, l.unidade)}</td>
  ...
  <div class="foot-bottom"><span class="ft-data">DATA&nbsp;&nbsp;_______ / _______ / _______</span><div class="sig-line"></div></div>
    ...
    return `<div class="page">${via}<div class="div-line"></div>${via}</div>`
  }).join('\n')
...
* { margin: 0; padding: 0; box-sizing: border-box; }
body { font-family: Arial, sans-serif; font-size: 10pt; background: ${preview ? '#e5e7eb' : '#fff'}; }
.toolbar { display: flex; align-items: center; gap: 8px; padding: 8px 14px; background: #1e293b; position: sticky; top: 0; z-index: 10; }
.btn-print { padding: 6px 18px; background: #16a34a; color: #fff; border: none; border-radius: 4px; font-size: 13px; font-weight: bold; cursor: pointer; }
.btn-print:hover { background: #15803d; }
.btn-close { padding: 6px 14px; background: #475569; color: #fff; border: none; border-radius: 4px; font-size: 13px; cursor: pointer; }
.btn-close:hover { background: #334155; }
.batch-info { margin-left: auto; color: #94a3b8; font-size: 12px; }
.page-wrap { padding: ${preview ? '12px' : '0'}; display: flex; flex-direction: column; gap: ${preview ? '20px' : '0'}; }
.page { display: flex; flex-direction: row; gap: 0; background: #fff; width: ${preview ? '277mm' : '100%'}; height: ${preview ? '190mm' : '100%'}; ${preview ? 'margin: 0 auto;' : ''} page-break-after: always; break-after: page; }
...
.sig-line { flex: 1; padding-top: 20mm; border-bottom: 1px solid #000; }
.div-line { border-left: 1px dashed #999; align-self: stretch; }
@media print { @page { size: A4 landscape; margin: 0; } body { background: #fff; } .toolbar { display: none; } .page-wrap { padding: 0; gap: 0; } .page { width: 297mm; height: 210mm; flex-direction: row; margin: 0 !important; } .via { width: 50%; height: 210mm; padding: 8mm 9mm; } }
</style>
</head>
<script>document.addEventListener('keydown', function(e){ if(e.key==='Escape') window.close(); });</script>
<body>
${preview ? `<div class="toolbar"><button class="btn-print" onclick="window.print()">🖨️ Imprimir Tudo (${dataArray.length})</button><button class="btn-close" onclick="window.close()">✕ Fechar</button><span class="batch-info">${dataArray.length} pedido${dataArray.length === 1 ? '' : 's'} para impressão</span></div>` : ''}
<div class="page-wrap">
${pages}
</div>
</body>
